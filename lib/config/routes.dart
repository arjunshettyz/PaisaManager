...
Box<dynamic> get settings {
  // Try to get from GetIt first (after DI initialization)
  try {
    if (getIt.isRegistered<Box<dynamic>>(instanceName: BoxType.settings.name)) {
      return getIt<Box<dynamic>>(instanceName: BoxType.settings.name);
    }
  } catch (e) {
    // GetIt not ready, fall through to Hive
  }
  // Fallback to Hive.box (should work after Hive.initFlutter)
  return Hive.box(BoxType.settings.name);
}
